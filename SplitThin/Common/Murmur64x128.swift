...
/// MurmurHash3 x64_128 — returns the first UInt64 of the 128-bit hash.
/// Used for bitmap key lookups in bounded fetch requests.
/// Ported from `org.apache.commons.codec.digest.MurmurHash3`.
enum Murmur64x128 {
    ...
    static func hashKey(_ key: String) -> UInt64 {
        hash(data: Array(key.utf8), offset: 0, length: UInt32(key.count), seed: 0)[0]
    }
    ...
    static func hash(data: [UInt8], offset: Int32, length: UInt32, seed: UInt64) -> [UInt64] {
        ...
        body(h1: &h1, h2: &h2, data: data, offset: offset, nblocks: nblocks)
    ...
    private static func body(h1: inout UInt64, h2: inout UInt64, data: [UInt8], offset: Int32, nblocks: Int32) {
        for i in 0..<nblocks {
...
extension UInt64 {
    fileprivate static let kSize: UInt64 = 64
