...
final class DefaultRetryableHttpClient: RetryableHttpClient, @unchecked Sendable {
    ...
    private func performRequest(endpoint: Endpoint, body: Data?) async throws -> HttpResponse {
        // HttpResponse comes from a dependency module not audited for Sendable, so it must be
        // boxed to cross into this continuation's closure without tripping strict concurrency checks.
        let boxed: UncheckedSendableBox<HttpResponse> = try await withCheckedThrowingContinuation { continuation in
            do {
